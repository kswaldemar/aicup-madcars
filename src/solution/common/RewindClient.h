...
#if defined(LOCAL_RUN) && defined(ENABLE_VISUALISER)
...
public:
    ...
    void circle(vec2 center, double r, uint32_t color, size_t layer = DEFAULT_LAYER) {
        circle(center.x, center.y, r, color, layer);
    }
    ...
    void line(vec2 p1, vec2 p2, uint32_t color, size_t layer = DEFAULT_LAYER) {
        line(p1.x, p1.y, p2.x, p2.y, color, layer);
    }
...
#define VIS_LINE(...) RewindClient::instance().line(__VA_ARGS__);
#define VIS_CIRCLE(...) RewindClient::instance().circle(__VA_ARGS__);
#define VIS_MESSAGE(...) RewindClient::instance().message(__VA_ARGS__);
#define VIS_END_FRAME() RewindClient::instance().end_frame();
